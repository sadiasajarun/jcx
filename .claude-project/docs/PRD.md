...
Issues identified while writing this PRD. Every item below is a **suggestion** — none is
specified in the client's source document, and none is adopted into the specification
above without client agreement.

## Suggestions

| # | Current Flow | Issue | Suggestion | Priority | Complexity |
|:-:|:------------|:------|:-----------|:--------:|:----------:|
| 1 | Sections 4, 5 and 6 each capture scroll — hijack, pin, pin | Three consecutive scroll-capturing sections. A visitor heading for the Map, Testimonials or Footer must play through all three before the page advances. On a trackpad this reads as a stuck page. | Release Section 5's pin after one Featured Project rather than all five, and add a persistent "skip section" affordance to each pinned region. | High | Medium |
| 2 | Landowner CTAs sit at the end of Section 6, behind three scroll-capturing sections | The JV offer is JCX's most differentiated product and success criterion #3 measures landowner engagement, yet a landowner must traverse six sections to reach it. | Add a landowner entry point above the fold — a second hero action, or a segment chooser in the utility strip that jumps to Section 6. | High | Simple |
| 3 | Filter state is set in the hero Search bar, applied in Sections 4 and 7 | A visitor filters, scrolls past two sections, and reaches a Projects strip already filtered with no visible cause. The filtered state has no explanation and no reset at the point of confusion. | Render an active-filter summary chip in the Projects command bar carrying a clear-all control. | High | Simple |
| 4 | Filter state lives in memory only | Reloading, or sharing the URL, silently discards the filter. The original brief specified `/properties?type=&status=&location=` — that query-string contract is absent from the homepage. | Mirror filter state into the query string and restore from it on load. | Medium | Simple |
| 5 | Search zero-match sets the button label to `No matches — Adjust filters` | The visitor is told the state but given no action. Matches known pattern `search_filter-001`. | Pair the message with a one-tap `Reset filters` control. | Medium | Simple |
| 6 | Custom cursors replace the native cursor inside Sections 4, 5, 6 and 8 | A label disc can obscure the affordance it replaces — the visitor loses the pointer/hand distinction that signals what is clickable. | Retain a small pointer indicator inside the disc, and confirm hover states remain independently legible. | Medium | Simple |
| 7 | Featured Projects binds `Enter` to open the project detail route | Navigating away mid-sequence discards the reel with no return path. | Open the project route in a new tab, or surface a detail panel in place. | Medium | Medium |
| 8 | WhatsApp FAB requires two taps — first reveals the tooltip, second opens the chat | Users expect a single tap on a chat button. The first tap reads as a failed tap. | Open the chat on first activation; show the tooltip on hover or after idle, never as a tap gate. | Medium | Simple |
| 9 | Awards (Section 9) has no inbound link | Reachable only by scrolling past eight sections. Credibility content that a business partner would look for directly. | Add Awards to the menu overlay. | Low | Simple |
| 10 | Every outbound CTA targets an unbuilt route | Within the prototype, `Explore`, `View all 60+ projects`, `Start your joint venture` and `Contact` are dead ends. During client review this reads as broken rather than as out-of-scope. | For the prototype only, point outbound CTAs at in-page anchors or render a "coming in the build phase" stub. Restore real routes for production. | High | Simple |

## Suggestion Details

### Suggestion 1 — Three consecutive scroll-capturing sections

- **Current:** Section 4 binds vertical scroll to horizontal translation. Section 5 pins until all five Featured Projects have played. Section 6 pins across five states.
- **Issue:** Combined, these hold the visitor for a long, non-skippable stretch. Section 5 alone is five Projects × four Acts × five seconds ≈ 100 seconds if watched passively. A visitor who wants the Map cannot tell how much remains.
- **Suggestion:** Release Section 5 after one Project rather than all five, and give every pinned region a visible progress indicator plus a skip control.
- **Expected benefit:** Protects success criterion #1 — that a visitor reaches the end of the page. This is the single largest bounce risk in the current flow.

### Suggestion 2 — Landowner path length

- **Current:** The JV offer appears only in Section 6, behind Sections 1–5.
- **Issue:** Landowners are a distinct audience with a distinct intent; the page presents homeowner content first and at length.
- **Suggestion:** Signpost the landowner path above the fold.
- **Expected benefit:** Directly serves success criterion #3.

### Suggestion 3 — Unexplained filtered state

- **Current:** Two controls write one filter state, separated by an intervening section.
- **Issue:** Cause and effect are separated by a full section of scrolling, so the effect appears uncaused.
- **Suggestion:** Show the active filter where its effect lands, with a reset.
- **Expected benefit:** Removes the most likely "the site is broken" moment during client review.

### Suggestion 10 — Dead-end CTAs during review

- **Current:** Outbound CTAs target routes built in a later engagement.
- **Issue:** The stated goal of this phase is written client sign-off. A reviewer clicking `Explore` and landing on nothing attributes the failure to the design.
- **Suggestion:** Neutralise outbound CTAs for the prototype build only.
- **Expected benefit:** Protects the sign-off itself — the phase's single gating criterion.
